...
def check(dir, prune):
    ...
    # lists of all png,pcap files for current dir
    png_files = [file for file in files if file.endswith(".png")]
    pcap_files = [file for file in files if file.endswith(".pcap")]

    if not png_files or not pcap_files:
    ...
    # make sure we have both png and pcap for this visit
    ...
    for png in png_files:
        ...
        if not is_ok(png_size, pcap_size, max_png_size, max_pcap_size, avg_png_size, avg_pcap_size):
            i += 1
            print(f"Error: {os.path.join(dir, png)} is not ok (png size: {png_size // 1024}Kb, max png size: {max_png_size //1024}Kb, avg png size: {avg_png_size // 1024}Kb, pcap size: {pcap_size // 1024}Kb, max pcap size {max_pcap_size // 1024}Kb, avg pcap size: {avg_pcap_size // 1024}Kb).")
...
def is_ok(png_size, pcap_size, max_png_size, max_pcap_size, avg_png_size, avg_pcap_size, threshold=0.5):
    min_size = 1024 * 50            # 50 KB minimum (besides avg)
    max_allowed = 1024 * 1500       # 1500 KB maximum (besides avg)

    def within_range(size, avg):
        return (avg * threshold <= size <= avg * (2 - threshold)) or (min_size <= size <= max_allowed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vpnlist", type=str, required=True, help="path to vpnlist.txt")
